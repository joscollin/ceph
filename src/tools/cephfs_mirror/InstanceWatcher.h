...
public:
  ...
  static InstanceWatcher *create(librados::IoCtx &ioctx,
                                 Listener &listener, ErrorListenerBase &elistener, ContextWQ *work_queue) {
    return new InstanceWatcher(ioctx, listener, elistener, work_queue);
  }

  InstanceWatcher(librados::IoCtx &ioctx, Listener &listener, ErrorListenerBase &elistener, ContextWQ *work_queue);
  ~InstanceWatcher();
